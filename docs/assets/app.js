(() => {
  ...
  const DEFAULT_SORT = {
    overview: ["估值區間(模型)", true], models: ["加權內在價值", false], bands: ["目前", true],
    metrics: ["ROE%", false], advice: ["AI推薦度", false],
  };
  const ALIGN_CENTER = new Set(["市場", "代號", "評級", "達安全邊際", "畫像", "幣別", "目前",
  ...
  function bandOf(r, ov) {
    const o = ov || ovOf(r);
    return o["估值區間(模型)"] || r["估值區間(模型)"] || o["估值區間"] || r["估值區間"] || r["目前"] || "";
  }

  function aiBandOf(r, ov) {
    const o = ov || ovOf(r);
    return o["估值區間(AI)"] || r["估值區間(AI)"] || o["AI區間"] || r["AI區間"] || "";
  }

  function headlineOf(ov, adv) {
    return ov["重中之重"] || (adv && adv["重中之重"]) || ov["一句話重點"]
      || (adv && adv["一句話重點(不考慮價格)"]) || "";
  }

  function adviceOf(adv) {
    return (adv && (adv["投資分析及建議"] || adv["200字投資建議"])) || "";
  }
  ...
  function colAlign(k) {
    if (ALIGN_CENTER.has(k) || String(k).includes("區間")) return "col-c";
    if (ALIGN_LEFT.has(k) || /重中之重|建議|警示|風險|假設|權重/.test(k)) return "col-l";
    return "col-r";
  }
  ...
  function tableHtml(rows, keys) {
    const head = `<th class="th-chart"></th>` + keys.map((k) =>
      `<th class="${colAlign(k)}" data-k="${esc(k)}">${esc(k)}${k === sortKey ? `<span class="arr">${sortAsc ? "↑" : "↓"}</span>` : ""}</th>`).join("");
    const body = rows.map((r, i) => `<tr class="row-link" data-i="${i}">
      ...
      ${keys.map((k) => `<td class="${colAlign(k)} ${toneClass(k, r[k])}">${cell(k, r[k])}</td>`).join("")}
    </tr>`).join("");
  ...
  function kvGrid(pairs) {
    const cells = pairs.filter((p) => p).map(([k, v, cls]) =>
  ...
  function cardBody(r, ov) {
    if (tab === "overview") {
      const note = headlineOf(ov, r);
      return kvGrid([
        ["內在價值", fmt(ov["內在價值"] ?? r["內在價值"])],
        ["折溢價", fmtPct(ov["折溢價%"], true), toneClass("折溢價%", ov["折溢價%"])],
        ["安全買點", fmt(ov["安全買點"])],
        ["達標", isBlank(ov["達安全邊際"]) ? "--" : ov["達安全邊際"], toneClass("達安全邊際", ov["達安全邊際"])],
        ["評級", isBlank(ov["評級"]) ? "--" : `<span class="stars">${esc(ov["評級"])}</span>`],
        ["推薦", ov["AI推薦度"] != null ? `${ov["AI推薦度"]}/10` : "--"],
        ["區間(AI)", bandLabel(aiBandOf(r, ov))],
      ]) + (note ? `<p class="card-note">${esc(note)}</p>` : "");
    }
    if (tab === "models") {
      const pairs = [["加權 IV", fmt(r["加權內在價值"])]];
      MODEL_COLS.forEach((k) => pairs.push([k.replace(/^[①②③④⑤⑥⑦⑧]/, ""), fmt(r[k])]));
      return kvGrid(pairs);
    }
    if (tab === "bands") {
      return kvGrid([
    ...
    if (tab === "advice") {
      const paras = splitAdvice(adviceOf(r) || headlineOf(ov, r));
      return `<div class="card-ai">
        <div class="card-ai-score"><span class="stars">${esc(r["評級"] || "")}</span>${r["AI推薦度"] != null ? `${r["AI推薦度"]}/10` : "--"}</div>
        <p class="band ${toneClass("估值區間(AI)", aiBandOf(r, ov))}">${bandLabel(aiBandOf(r, ov))}</p>
  ...
  function bandLabel(v) {
    const t = isBlank(v) ? "--" : String(v);
    const cls = isBlank(v) ? "" : esc(String(v));
    return `<span class="band ${toneClass("估值區間(模型)", v)}"><i class="dot ${cls}"></i>${esc(t)}</span>`;
  }

  function cell(k, v) {
    if (k.includes("區間") || k === "目前") return bandLabel(v);
    if (k === "評級") return isBlank(v) ? "--" : `<span class="stars">${esc(v)}</span>`;
    if (isBlank(v)) return "--";
  ...
  function toneClass(k, v) {
    ...
    if (k === "估值區間" || k === "目前" || k === "估值區間(模型)" || k === "估值區間(AI)" || k === "AI區間") {
  ...
  function openDetail(r) {
    ...
    const headline = headlineOf(ov, adv);
    const paras = splitAdvice(adviceOf(adv));
    const risks = adv["主要風險"] || ov["警示"] || "";
    const modelBand = ov["估值區間(模型)"] || ov["估值區間"] || "";
    const aiBand = ov["估值區間(AI)"] || ov["AI區間"] || adv["估值區間(AI)"] || "";
    $("#modalBody").innerHTML = `
      ...
      <div class="kv">
        ...
        <b>安全買點</b><span>${fmt(ov["安全買點"])}（MOS ${isBlank(ov["安全邊際MOS%"]) ? "--" : `${esc(ov["安全邊際MOS%"])}%`} · <span class="${toneClass("達安全邊際", ov["達安全邊際"])}">${esc(ov["達安全邊際"] || "--")}</span>）</span>
